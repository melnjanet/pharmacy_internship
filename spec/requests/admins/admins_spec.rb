...
RSpec.describe "Admins::Admins", type: :request do
  ...
  describe 'GET /show' do
    it 'renders a successful response' do
      get admins_admin_url(valid_admin)
      expect(response).to be_successful
    end
  end
  ...
  describe 'GET /edit' do
    it 'renders a successful response' do
      get edit_admins_admin_url(valid_admin)
      expect(response).to be_successful
    end
  end
  ...
  describe 'POST /create' do
    context 'with valid parameters' do
      it 'creates a new Admin' do
        expect { post admins_admins_url, params: { admin: valid_attributes } }.to change(Admin, :count).by(1)
      end

      it 'redirects to the created admin' do
        post admins_admins_url, params: { admin: valid_attributes }
    ...
    context 'with invalid parameters' do
      it 'does not create a new Admin' do
        expect { post admins_admins_url, params: { admin: invalid_attributes } }.to change(Admin, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post admins_admins_url, params: { admin: invalid_attributes }
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      ...
      it 'redirects to the admin' do
        patch admins_admin_url(valid_admin), params: { admin: new_attributes }
        valid_admin.reload
    ...
    context 'with invalid parameters' do
      it 'renders a successful response but not change valid admin' do
        patch admins_admin_url(valid_admin), params: { admin: invalid_attributes }
        valid_admin.reload
  ...
  describe 'DELETE /destroy' do
    ...
    it 'destroys the requested admin' do
      expect {
        delete admins_admin_url(admin_for_destroy)
      }.to change(Admin, :count).by(-1)
    end

    it 'redirects to the admins list' do
      delete admins_admin_url(valid_admin)

...
RSpec.describe "Recipes", type: :request do
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      ...
      it 'updates the requested recipe' do
        patch recipe_url(valid_recipe), params: { recipe: new_attributes }
        valid_recipe.reload
      ...
      it 'redirects to the recipe' do
        patch recipe_url(valid_recipe), params: { recipe: new_attributes }
        valid_recipe.reload
        expect(response).to redirect_to(recipe_url(valid_recipe))
      end
    end

    context 'with invalid parameters' do
      it 'renders a successful response but not change valid recipe' do
        patch recipe_url(valid_recipe), params: { recipe: invalid_attributes }
        valid_recipe.reload
  ...
  describe 'DELETE /destroy' do
    ...
    it 'destroys the requested recipe' do
      expect {
        delete recipe_url(recipe_for_destroy)
      }.to change(Recipe, :count).by(-1)
    end

    it 'redirects to the recipe list' do
      delete recipe_url(valid_recipe)

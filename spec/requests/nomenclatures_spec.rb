...
RSpec.describe "Nomenclatures", type: :request do
  ...
  describe 'GET /show' do
    it 'renders a successful response' do
      get nomenclature_url(valid_nomenclature)
      expect(response).to be_successful
    end
  end
  ...
  describe 'GET /edit' do
    it 'renders a successful response' do
      get edit_nomenclature_url(valid_nomenclature)
      expect(response).to be_successful
    end
  end
  ...
  describe 'POST /create' do
    context 'with valid parameters' do
      it 'creates a new Nomenclature' do
        expect { post nomenclatures_url, params: { nomenclature: valid_attributes } }.to change(Nomenclature, :count).by(1)
      end

      it 'redirects to the created nomenclature' do
        post nomenclatures_url, params: { nomenclature: valid_attributes }
        expect(response).to redirect_to(nomenclature_url(Nomenclature.last))
      end
    end

    context 'with invalid parameters' do
      it 'does not create a new Nomenclature' do
        expect { post nomenclatures_url, params: { nomenclature: invalid_attributes } }.to change(Nomenclature, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post nomenclatures_url, params: { nomenclature: invalid_attributes }
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      ...
      it 'updates the requested nomenclature' do
        patch nomenclature_url(valid_nomenclature), params: { nomenclature: new_attributes }
      ...
      it 'redirects to the nomenclature' do
        patch nomenclature_url(valid_nomenclature), params: { nomenclature: new_attributes }
        valid_nomenclature.reload
        expect(response).to redirect_to(nomenclature_url(valid_nomenclature))
      end
    end

    context 'with invalid parameters' do
      it 'renders a successful response but not change valid nomenclature' do
        patch nomenclature_url(valid_nomenclature), params: { nomenclature: invalid_attributes }
        valid_nomenclature.reload
  ...
  describe 'DELETE /destroy' do
    ...
    it 'destroys the requested nomenclature' do
      expect {
        delete nomenclature_url(nomenclature_for_destroy)
      }.to change(Nomenclature, :count).by(-1)
    end

    it 'redirects to the nomenclature list' do
      delete nomenclature_url(valid_nomenclature)
      expect(response).to redirect_to(nomenclatures_url)
    end
  end
end

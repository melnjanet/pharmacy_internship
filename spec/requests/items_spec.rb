...
RSpec.describe "Items", type: :request do
  ...
  describe 'POST /create' do
    ...
    context 'with invalid parameters' do
      it 'does not create a new Item' do
        expect { post items_url, params: { item: invalid_attributes } }.to change(Item, :count).by(0)
      end
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      ...
      it 'updates the requested item' do
        patch item_url(valid_item), params: { item: new_attributes }
        valid_item.reload
      ...
      it 'redirects to the item' do
        patch item_url(valid_item), params: { item: new_attributes }
        valid_item.reload
        expect(response).to redirect_to(item_url(valid_item))
      end
    end

    context 'with invalid parameters' do
      it 'renders a successful response but not change valid item' do
        patch item_url(valid_item), params: { item: invalid_attributes }
        valid_item.reload
  ...
  describe 'DELETE /destroy' do
    ...
    it 'destroys the requested item' do
      expect {
        delete item_url(item_for_destroy)
      }.to change(Item, :count).by(-1)
    end

    it 'redirects to the item list' do
      delete item_url(valid_item)

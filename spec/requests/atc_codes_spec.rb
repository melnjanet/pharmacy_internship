...
RSpec.describe "ATC Codes", type: :request do
  ...
  describe 'GET /show' do
    it 'renders a successful response' do
      get atc_code_url(valid_atc_code)
      expect(response).to be_successful
    end
  end
  ...
  describe 'GET /edit' do
    it 'renders a successful response' do
      get edit_atc_code_url(valid_atc_code)
      expect(response).to be_successful
    end
  end
  ...
  describe 'POST /create' do
    context 'with valid parameters' do
      it 'creates a new ATC Code' do
        expect { post atc_codes_url, params: { atc_code: valid_attributes } }.to change(AtcCode, :count).by(1)
      end

      it 'redirects to the created atc_code' do
        post atc_codes_url, params: { atc_code: valid_attributes }
        expect(response).to redirect_to(atc_code_url(AtcCode.last))
      end
    end

    context 'with invalid parameters' do
      it 'does not create a new ATCC Code' do
        expect { post atc_codes_url, params: { atc_code: invalid_attributes } }.to change(AtcCode, :count).by(0)
      end

      it "renders a successful response (i.e. to display the 'new' template)" do
        post atc_codes_url, params: { atc_code: invalid_attributes }
  ...
  describe 'PATCH /update' do
    context 'with valid parameters' do
      ...
      it 'updates the requested atc_code' do
        patch atc_code_url(valid_atc_code), params: { atc_code: new_attributes }
      ...
      it 'redirects to the atc_code' do
        patch atc_code_url(valid_atc_code), params: { atc_code: new_attributes }
        valid_atc_code.reload
        expect(response).to redirect_to(atc_code_url(valid_atc_code))
      end
    end

    context 'with invalid parameters' do
      it 'renders a successful response but not change valid atc_code' do
        patch atc_code_url(valid_atc_code), params: { atc_code: invalid_attributes }
        valid_atc_code.reload
  ...
  describe 'DELETE /destroy' do
    ...
    it 'destroys the requested atc_code' do
      expect {
        delete atc_code_url(atc_code_for_destroy)
      }.to change(AtcCode, :count).by(-1)
    end

    it 'redirects to the atc_codes list' do
      delete atc_code_url(valid_atc_code)
      expect(response).to redirect_to(atc_codes_url)
    end
  end
end
